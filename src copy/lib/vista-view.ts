...
export class VistaView {
  ...
  private zoomInEnabled: boolean = true;
  async zoomIn(): Promise<void> {
    if (!this.zoomInEnabled) return;
    this.zoomInEnabled = false;
    await this.imgStabilized;
    this.imgState.setInitCentroid({
    ...
    state.image?.addEventListener(
      'transitionend',
      () => {
        this.imgStabilized = this.imgState.stabilizeProps();
        this.imgStabilized.then(() => {
  ...
  private zoomOutEnabled: boolean = true;
  async zoomOut(): Promise<void> {
    if (!this.zoomOutEnabled) return;
  ...
  open(startIndex: number = 0): void {
    ...
    this.rootElm.querySelector('.vvw-close')?.addEventListener('click', () => this.close());
    this.rootElm.querySelector('.vvw-zoom-in')?.addEventListener('click', () => this.zoomIn());
    this.rootElm.querySelector('.vvw-zoom-out')?.addEventListener('click', () => this.zoomOut());
    this.rootElm.querySelector('.vvw-prev>button')?.addEventListener('click', () => this.prev());
    this.rootElm.querySelector('.vvw-next>button')?.addEventListener('click', () => this.next());
