...
We provide two separate refs: **`containerRef`** for the DOM element and **`vistaRef`** for the imperative API. Passing refs as props keeps responsibilities clear and types safe.
...
const vistaRef = ref(null);
const containerRef = ref(null);

const openGallery = () => {
  vistaRef.value?.open(0);
  // DOM element available at containerRef.value
};
</script>

<template>
  <VistaView :vistaRef="vistaRef" :containerRef="containerRef" selector="> a">
...
> Note: using `ref` on the component returns an object with `vista` (the API) and `container` (the root DOM element). Example: `compRef.value.vista.open(0)` and `compRef.value.container`.
