...
onMount(async () => {
  wistiaThumb = await getWistiaThumbnail(wistiaVideoUrl);
  images = images.map(img =>
    img.full === wistiaVideoUrl ? { ...img, thumb: wistiaThumb } : img
  );
});
</script>

<style>
:global(.vistaviewGrid) {
  display: grid;
...
<main style="padding: 32px">
  <h1>VistaView Svelte - Extension Demo</h1>
  <VistaView selector="> a" options={vistaOptions} class="vistaviewGrid">
    {#each images as img, i}
      <a
        href={img.full}
        {...(img.srcset ? { 'data-vistaview-srcset': img.srcset } : {})}
