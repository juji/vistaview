...
const GOOGLE_MAPS_API_KEY = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || 'asdf';
const MAPBOX_ACCESS_TOKEN = process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN || 'asdf';
...
export default function ExtensionDemo() {
  const [wistiaThumb, setWistiaThumb] = useState("");

  // Build images array with current wistiaThumb
  const images = [
  ...
  const fetchWistiaThumb = useCallback(async () => {
  ...
  return (
    <main style={{ padding: "32px" }}>
      <h1>VistaView Next.js - Extension Demo</h1>
      <VistaView
        selector={"> a"}
        className={styles.vistaviewGrid}
        options={{
          controls: {
      ...
      >
          ...
          {images.map((img, i) => (
            <a
              ...
              target="_blank"
              className={styles.vistaviewAnchor}
            >
              <img src={img.thumb} alt={img.alt} loading="lazy" className={styles.vistaviewThumb} />
            </a>
          ))}
      </VistaView>
